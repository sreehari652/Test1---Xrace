"""
UWB Racing Tracker - Matplotlib Version
Main racing application using matplotlib instead of pygame
...
Controls (type in console and press Enter):
    ...
    r or reset     - Reset race
    s or stats     - Print full statistics
    l or laps      - Print lap times
    c or collisions - Print collision report
    p or points    - Print points leaderboard
"""
...
# Import matplotlib renderer
from matplotlib_renderer import MatplotlibRenderer


class RacingTrackerMatplotlib:
    """Main racing application class using matplotlib"""

    def __init__(self):
        """Initialize the UWB racing tracking system"""
        print("\n" + "=" * 60)
        print("🏎️  UWB RACING TRACKER - Matplotlib Edition")
        print("=" * 60)
        ...
        self.renderer = MatplotlibRenderer(scale_params)
    ...
    def _print_controls(self):
        ...
        print("\n" + "=" * 60)
        print("Controls (type command and press Enter):")
        print("  q or quit      - Quit application")
        print("  r or reset     - Reset race")
        print("  s or stats     - Print full statistics")
        print("  l or laps      - Print lap times")
        print("  c or collisions - Print collision report")
        print("  p or points    - Print points leaderboard")
        print("=" * 60 + "\n")

    def _command_loop(self):
        """Background thread for handling commands"""
        while self.running:
            try:
                ...
                elif cmd in ['c', 'collisions']:
                    self.print_collision_report()
                    
                elif cmd in ['p', 'points']:
    ...
    def print_lap_times(self):
        ...
        for car_id, lap_info in all_lap_info.items():
            ...
            for idx, lap_time in enumerate(lap_info['lap_times']):
                lap_num = idx + 1
                marker = " (BEST)" if lap_time == lap_info['best_lap_time'] else ""
    ...
    def print_points_leaderboard(self):
        ...
        for idx, car_data in enumerate(leaderboard):
            ...
            print(f"{position}. {car_data['car_name']}: {car_data['points']:+4d} points "
    ...
    def run(self):
        ...
        print("\n🏁 Starting race tracker...")
        print("Type commands and press Enter to control the application.\n")
...
def main():
    """Entry point"""
    tracker = RacingTrackerMatplotlib()
    tracker.run()
